...
<h1 class="page-header text-center">ANNUAL REPORTS</h1>
...
<table class="table table-striped table-bordered">
		<thead>
			...
			<th>TOTAL</th>
<?php 
			$sql = "select YEAR(date_purchase),count(customer),sum(total) from purchase group by YEAR(date_purchase)";
				$query=$conn->query($sql);
				while($row=$query->fetch_array()){
					?>
					<tr>
						<td><?php echo $row['YEAR(date_purchase)']; ?></td>
...
<button type="submit" class="btn btn-default" data-dismiss="modal"><span class="glyphicon glyphicon-remove"></span><a href="index.php">Close</a></button></div> 
</div>
