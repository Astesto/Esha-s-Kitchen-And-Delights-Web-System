...
<h1 class="page-header text-center">WEEKLY REPORTS</h1>
...
<table class="table table-striped table-bordered">
		<thead>
			...
			<th>TOTAL</th>
<?php 
				$sql = "select WEEK(date_purchase),count(customer),sum(total) from purchase group by WEEK(date_purchase)";
				$query=$conn->query($sql);
				while($row=$query->fetch_array()){
					?>
					<tr>
						<td><?php echo $row['WEEK(date_purchase)']; ?></td>
