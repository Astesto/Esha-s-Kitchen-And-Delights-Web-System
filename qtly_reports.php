...
<h1 class="page-header text-center">QUARTERLY REPORTS</h1>
...
<table class="table table-striped table-bordered">
		<thead>
			<th>NUMBER 	OF QUARTER</th>
			<th>TOTAL NUMBER OF CUSTOMERS</th>
			<th>TOTAL</th>
<?php 
			$sql = "select QUARTER(date_purchase),count(customer),sum(total) from purchase group by QUARTER(date_purchase)";
				$query=$conn->query($sql);
				while($row=$query->fetch_array()){
					?>
					<tr>
						<td><?php echo $row['QUARTER(date_purchase)']; ?></td>
